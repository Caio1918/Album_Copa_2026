...
type IconName = 'Home' | 'BookOpen' | 'Repeat2'
...
function renderIcon(name: IconName) {
  const paths = {
    Home: '<path d="m3 9 9-7 9 7"/><path d="M9 22V12h6v10"/><path d="M21 9v13H3V9"/>',
    BookOpen: '<path d="M12 7v14"/><path d="M3 18a1 1 0 0 0 1 1h6a2 2 0 0 1 2 2V7a2 2 0 0 0-2-2H4a1 1 0 0 0-1 1z"/><path d="M21 18a1 1 0 0 1-1 1h-6a2 2 0 0 0-2 2V7a2 2 0 0 1 2-2h6a1 1 0 0 1 1 1z"/>',
    Repeat2: '<path d="m17 2 4 4-4 4"/><path d="M3 11v-1a4 4 0 0 1 4-4h14"/><path d="m7 22-4-4 4-4"/><path d="M21 13v1a4 4 0 0 1-4 4H3"/>',
  }
  return `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round">${paths[name]}</svg>`
}

function applyIcons() {
  const items = Array.from(document.querySelectorAll<HTMLButtonElement>('.bottom-item'))
  const iconNames: IconName[] = ['Home', 'BookOpen', 'Repeat2']
...
function applyFilterSelect() {
  ...
  bar.innerHTML = `<div><span>Filtro</span><strong>${active.textContent === 'todas' ? 'Todas as figurinhas' : active.textContent || 'Todas'}</strong></div><select aria-label="Selecionar filtro de figurinhas">${options.map((button) => `<option value="${button.textContent || ''}" ${button === active ? 'selected' : ''}>${button.textContent || ''}</option>`).join('')}</select>`
